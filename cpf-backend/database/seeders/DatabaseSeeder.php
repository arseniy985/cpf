...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ([
            [1, 'Инвестиции', 'Как проходит регистрация и когда открывается кабинет?', 'После создания аккаунта вы подтверждаете email кодом и сразу попадаете в личный кабинет. Там доступны проекты, документы, заявки и история действий.'],
            [2, 'Инвестиции', 'С какой суммы можно начать и где видеть статус участия?', 'Минимальный чек зависит от проекта, но базовые предложения стартуют от 10 000 ₽. Статус заявки, документы и история действий отображаются прямо в кабинете инвестора.'],
            [3, 'Инвестиции', 'Как происходят выплаты и вывод средств?', 'Доход и возвраты отображаются в личном кабинете. Если доступен вывод, вы подаете заявку, а ее статус и комментарии команды видите там же.'],
            [4, 'Сделки', 'Как платформа проверяет проекты перед публикацией?', 'Команда анализирует юридическую структуру, финансовую модель, документы по активу и ключевые риски. На витрину попадают только подготовленные проекты с понятным пакетом раскрытия.'],
            [5, 'Сделки', 'Что получает владелец объекта после регистрации?', 'Владелец получает отдельный кабинет для заполнения данных компании, загрузки документов, добавления объекта и публикации отчетов. Публичный сайт и рабочий кабинет разделены.'],
            [6, 'Сделки', 'Можно ли обновлять FAQ, цифры и отзывы через CMS?', 'Да, тексты, вопросы, отзывы и основные цифры можно менять через CMS. Пока редактор не заполнил разделы, платформа показывает стартовые данные по умолчанию.'],
        ] as [$sortOrder, $group, $question, $answer]) {
        ...
        foreach ([
            ...
            [
                ...
                'summary' => 'Инвестор регистрируется, подтверждает личность, пополняет баланс и выбирает проект.',
        ...
        foreach ([
            [
                'author_name' => 'Алексей Миронов',
                'author_role' => 'Частный инвестор',
                'company_name' => null,
                'rating' => 5,
                'body' => 'Понравилось, что после регистрации сразу видно структуру сделки, документы и статус заявки. Не пришлось вести переписку в нескольких каналах.',
                'sort_order' => 1,
            ],
            [
                'author_name' => 'Елена Воронова',
                'author_role' => 'Финансовый директор SPV',
                'company_name' => 'North Estate',
                'rating' => 5,

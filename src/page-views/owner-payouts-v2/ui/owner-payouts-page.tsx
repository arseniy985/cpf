  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
        <AppKpiCard label="Всего выплат" value={String(payouts.length)} hint="Все выплаты, созданные в текущем разделе." />
      ...
      <AppSurface eyebrow="Реестр" title="Реестр выплат" description="Список выплат с текущими статусами, замечаниями и результатами обработки.">
        {payouts.length ? (
            ...
            {payouts.map((payout) => (
              <div key={payout.id} className="border border-app-cabinet-border bg-app-cabinet-surface px-4 py-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <p className="text-sm font-semibold text-app-cabinet-text">{payout.referenceLabel ?? payout.distributionTitle ?? 'Выплата владельцу'}</p>
          ...
          <AppEmptyState title="Выплат пока нет" description="Когда начнутся распределения по раундам, здесь появится реестр выплат." />
        )}
      </AppSurface>
    </div>
  );

...
import { ArrowRight, Download, FileText, Filter, History } from 'lucide-react';
...
type FilterValue = 'all' | 'approved' | 'pending' | 'completed';
...
export default function InvestorPortfolioPage() {
  ...
  const rows = filter === 'all'
    ? investments
    : investments.filter((item) => item.status === filter);
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div className="flex gap-3">
          <Button variant="outline" className="gap-2 border-slate-200 bg-white text-brand-primary hover:bg-brand-secondary">
      ...
      <div className="flex flex-col items-center justify-between gap-4 rounded-sm border border-[#E2E8F0] bg-white p-4 sm:flex-row">
        <div className="flex w-full items-center gap-2 overflow-x-auto pb-2 sm:w-auto sm:pb-0">
          <button type="button" onClick={() => setFilter('all')} className="cursor-pointer whitespace-nowrap rounded-full border border-transparent bg-brand-primary px-3 py-1 text-xs font-semibold text-white">
            Все ({investments.length})
          </button>
          <button type="button" onClick={() => setFilter('approved')} className="cursor-pointer whitespace-nowrap rounded-full border border-slate-200 px-3 py-1 text-xs font-semibold text-brand-text">
            Активные ({investments.filter((item) => item.status === 'approved' || item.status === 'confirmed').length})
          </button>
          <button type="button" onClick={() => setFilter('pending')} className="cursor-pointer whitespace-nowrap rounded-full border border-slate-200 px-3 py-1 text-xs font-semibold text-brand-text">
            Заявки ({investments.filter((item) => item.status === 'pending').length})
          </button>
          <button type="button" onClick={() => setFilter('completed')} className="cursor-pointer whitespace-nowrap rounded-full border border-slate-200 px-3 py-1 text-xs font-semibold text-brand-text">
            Завершенные ({investments.filter((item) => item.status === 'completed').length})
          </button>
        </div>
        <Button variant="ghost" size="sm" className="w-full justify-start gap-2 text-brand-text-muted sm:w-auto sm:justify-center">
                ...
                {rows.map((item) => (
                  <tr key={item.id} className="transition-colors hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <AppStatusBadge status={item.status === 'confirmed' ? 'approved' : item.status} className={item.status === 'approved' || item.status === 'confirmed' ? 'bg-brand-success/10 text-brand-success' : 'bg-brand-warning/10 text-brand-warning'} />
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button asChild variant="ghost" size="icon">
                          <Link href="/app/investor/portfolio">
                ...
                <div className="space-y-4 p-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button asChild className="h-9 w-full border border-brand-primary bg-brand-primary text-xs text-white hover:bg-brand-primary/90">
                      <Link href="/app/investor/portfolio">Подробнее</Link>
                    </Button>
                  </div>
                </div>
              </div>

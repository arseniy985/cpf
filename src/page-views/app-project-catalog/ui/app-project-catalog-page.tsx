...
export default function AppProjectCatalogPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-[#DCE6F5] bg-[linear-gradient(180deg,#f8fbff_0%,#ffffff_100%)] shadow-[0_18px_40px_rgba(18,49,102,0.08)]">
        <div className="border-b border-[#DCE6F5] px-5 py-5 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div className="space-y-4">
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-text-muted">Подбор проектов</p>
                <h2 className="text-2xl font-semibold tracking-tight text-brand-text sm:text-3xl">Поиск и фильтры каталога</h2>
                <p className="max-w-2xl text-sm leading-6 text-brand-text-muted">
                  Начните с параметров сделки: название или локация, тип актива, размер входа и доходность.
                </p>
              </div>
              ...
              <label className="block max-w-2xl">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="h-12 rounded-full border-[#C9D8EE] bg-white pl-11 shadow-none"
                  />
                </div>
              </label>
            </div>

            <div className="grid gap-3 sm:grid-cols-2 xl:min-w-[20rem] xl:max-w-[22rem] xl:grid-cols-1">
              <CatalogStatCard label="Найдено" value={String(visibleProjects.length)} note="проектов по текущим параметрам" />
              <CatalogStatCard
                label="Тип актива"
                value={category === 'Все' ? 'Все' : projectCategoryLabels[category]}
                note="основной фильтр каталога"
              />
            </div>
          </div>
        </div>
              ...
              {categories.map((item) => (
                <Button
                  ...
                  className={category === item
                    ...
                    : 'rounded-full border-[#C9D8EE] bg-white px-4 text-app-cabinet-text hover:bg-brand-secondary'}
            ...
            visibleProjects.length ? (
              <div className="grid gap-4 xl:grid-cols-2">
                {visibleProjects.map((project) => (
                  <article
                    key={project.id}
                    className="overflow-hidden rounded-[2rem] border border-[#DCE6F5] bg-white shadow-[0_14px_28px_rgba(18,49,102,0.06)]"
                  >
...
function CatalogStatCard({
  ...
  return (
    <div className="rounded-[1.5rem] border border-[#D7E3F3] bg-[linear-gradient(180deg,#f2f7ff_0%,#ffffff_100%)] p-4">
      ...
      <p className="mt-2 text-sm text-brand-text-muted">{note}</p>
    </div>
  );
}

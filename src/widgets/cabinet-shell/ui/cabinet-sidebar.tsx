...
export function CabinetSidebar({
  ...
  return (
    <div className="flex h-full flex-col bg-cabinet-panel px-4 pb-4 pt-5">
      <div>
        <CabinetBrand
          className="border border-cabinet-border/80 bg-cabinet-panel-strong px-4 py-3"
          onClick={onNavigate}
        />
      </div>

      <div className="pt-4">
        <div className="rounded-[16px] border border-cabinet-border/80 bg-cabinet-panel-strong p-4 shadow-[0_12px_28px_rgba(17,35,63,0.04)]">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[12px] bg-cabinet-ink text-sm font-semibold text-cabinet-panel-strong">
            ...
            <div key={group.id}>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      onClick={onNavigate}
                      className={cn(
                        'group flex items-center gap-3 rounded-[14px] border px-3 py-3 transition-[background-color,border-color,color,transform] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cabinet-accent/30',
                        isActive
                          ? 'border-cabinet-ink bg-cabinet-ink text-cabinet-panel-strong shadow-[0_12px_24px_rgba(17,35,63,0.16)]'
              ...
              {group.id === 'investor' ? (
                <p className="px-3 pt-3 text-xs leading-relaxed text-cabinet-muted-ink">
                  Деньги, статус проверки и ключевые действия собраны в одном кабинете.
                </p>
              ) : null}
            </div>
          ))}
      ...
      <div className="pt-4">
        ...
        <div className="mt-4 space-y-2">
            ...
            return (
              <Link key={item.href} href={item.href} onClick={onNavigate}>
                <Button
                  variant="outline"
                  width="full"
                  className="justify-start rounded-[14px] border-cabinet-border bg-cabinet-panel-strong text-cabinet-ink hover:bg-cabinet-accent-soft"
                >
                  <Icon className="h-4 w-4" />
                  {item.label}
                </Button>
          ...
          <Button
            variant="ghost"
            width="full"
            className="justify-start rounded-[14px] text-cabinet-muted-ink hover:bg-cabinet-panel-strong hover:text-cabinet-ink"
